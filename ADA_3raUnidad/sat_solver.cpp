/**
   * Codigo extraido del siguiente repositorio: https://en.wikipedia.org/wiki/DPLL_algorithm.
**/
...
class belive_rev{
    ...
    unsigned int numVariables2;

    /**
     * The number of clauses of the formula of the satisfiability problem.
     */
    unsigned int numClauses2;
     ...
     * The occurrence list of positive appearances for each value in the clause set.
     */
    vector<vector<vector<int>* > > positiveClauses2;

    /**
     * The occurrence list of negative appearances for each value in the clause set.
     */
    vector<vector<vector<int>* > > negativeClauses2;
     ...
     * The stack that tracks the current execution state (the backtrack stack).
     */
    vector<int> modelStack2;

    /**
     * An index indicating which is the next literal from the stack to be propagated.
     */
    unsigned int  indexOfNextLiteralToPropagate2;

    /**
     * The current decision level of the DPLL algorithm.
     */
    unsigned int  decisionLevel2;

    /**
     * The activity (number of conflicts in which appears) for each positive literal.
     */
    vector<double> positiveLiteralActivity2;

    /**
     * The activity (number of conflicts in which appears) for each negative literal.
     */
    vector<double> negativeLiteralActivity2;

    /**
     * The total number of conflicts found during the DPLL execution.
     */
    unsigned int  conflicts2;

    /**
     * The total number of propagations performed by the DPLL algorithm.
     */
    unsigned int  propagations2;

    /**
     * The total number of literal decisions taken by the DPLL algorithm.
     */
    unsigned int  decisions2;
    /**
    ...
    void exitWithSatisfiability(bool satisfiable) {
        ...
        if (satisfiable==true) {
            checkmodel();
            cout << "SATISFIABLE," <<endl;//<< decisions << "," << propagations << endl;
            
            
        }
        else {
            cout << "UNSATISFIABLE," << endl;//decisions << "," << propagations << endl;
        }
    }
    ...
    int currentValueForLiteral2(int literal) {
        if (literal >= 0) {
            return model2[literal];
        }
        else {
            if (model2[-literal] == UNDEFINED) {
                return UNDEFINED;
            }
            else {
                return 1 - model2[-literal];
            }
        }
    }
     ...
     * @param literal the literal that will become true after the model update
     */
    void setLiteralToTrue2(int literal) {
        modelStack2.push_back(literal);
        if (literal > 0) {
            model2[literal] = TRUE;
        }
        else {
            model2[-literal] = FALSE;
        }
    }
     ...
     * @param literal the literal which activity is to be updated
     */
    void updateActivityForLiteral2(int literal) {
        //update the activity of the literal (we are not distinguishing between positive
        // and negative literals here)
        unsigned int  index = var(literal);
        if (literal > 0) {
            positiveLiteralActivity2[index] += ACTIVITY_INCREMENT;
        }
        else {
            negativeLiteralActivity2[index] += ACTIVITY_INCREMENT;
        }
    }
     ...
     * @param clause the clause which was involved in the most recent conflict
     */
    void updateActivityForConflictingClause2(const vector<int>& clause) {
        //update the activity increment if necessary (every X conflicts)
        ++conflicts;
        if ((conflicts % ACT_INC_UPDATE_RATE) == 0) {
            //decaying sum
            for (unsigned int  i = 1; i <= numVariables2; ++i) {
                positiveLiteralActivity2[i] /= 2.0;
                negativeLiteralActivity2[i] /= 2.0;
            }
        }

        //update activity for each literal
        for (unsigned int  i = 0; i < clause.size(); ++i) {
            updateActivityForLiteral2(clause[i]);
        }
    }
    ...
    bool propagateGivesConflict2() {
        while (indexOfNextLiteralToPropagate2 < modelStack2.size()) {
            //retrieve the literal to "be propagated"
            int literalToPropagate = modelStack2[indexOfNextLiteralToPropagate];
            //move forward the "pointer" to the next literal that will be propagated
            ++indexOfNextLiteralToPropagate2;

            ++propagations2; //profiling purposes only

            //traverse only positive/negative appearances
            vector<vector<int>* > clausesToPropagate2 = literalToPropagate > 0 ?
                    negativeClauses2[var(literalToPropagate)] :
                    positiveClauses2[var(literalToPropagate)];

            //traverse the clauses
            for (unsigned int  i = 0; i < clausesToPropagate2.size(); ++i) {
                //retrieve the next clause
                vector<int> clause2 = (vector<int>) *clausesToPropagate2[i];
                ...
                //traverse the clause
                for (unsigned int  k = 0; not isSomeLiteralTrue and k < clause2.size(); ++k) {
                    int value = currentValueForLiteral2(clause2[k]);
                    if (value == TRUE) {
                        isSomeLiteralTrue = true;
                    }
                    else if (value == UNDEFINED) {
                        ++undefinedLiterals;
                        lastUndefinedLiteral = clause2[k];
                    }
                }
                if (not isSomeLiteralTrue and undefinedLiterals == 0) {
                    // A conflict has been found! All literals are false!
                    updateActivityForConflictingClause2(clause2);
                    return true;
                }
                else if (not isSomeLiteralTrue and undefinedLiterals == 1) {
                    // The 'lastUndefinedLiteral' is "propagated", because it is the only one
                    ...
                    //  of the clause are false
                    // With the propagation, this literal is added to the stack, too!
                    setLiteralToTrue2(lastUndefinedLiteral);
                }
            }
        }
        return false;
     ...
     */
    void backtrack2() {
        unsigned int  i = modelStack2.size() - 1;
        int literal = 0;
        while (modelStack2[i] != DECISION_MARK) { // 0 is the  mark
            literal = modelStack2[i];
            model2[var(literal)] = UNDEFINED;
            modelStack2.pop_back();
            --i;
        }
        // at this point, literal is the last decision
        modelStack2.pop_back(); // remove the  mark
        --decisionLevel2;
        indexOfNextLiteralToPropagate2 = modelStack2.size();
        setLiteralToTrue2(-literal);  // reverse last decision
    }
    ...
    int getNextDecisionLiteral2() {
        ...
        for (unsigned int  i = 1; i <= numVariables2; ++i) {
            // check only undefined variables
            if (model2[i] == UNDEFINED) {
                // search for the most active literal
                if (positiveLiteralActivity2[i] >= maximumActivity) {
                    maximumActivity = positiveLiteralActivity2[i];
                    mostActiveVariable = i;
                }
                if (negativeLiteralActivity2[i] >= maximumActivity) {
                    maximumActivity = negativeLiteralActivity2[i];
                    mostActiveVariable = -i;
                }
            }
        }
     ...
     */
    void checkmodel2() {
        for (unsigned int  i = 0; i < numClauses2; ++i) {
            bool someTrue = false;
            for (unsigned int  j = 0; not someTrue and j < clauses2[i].size(); ++j) {
                someTrue = (currentValueForLiteral2(clauses2[i][j]) == TRUE);
            }
            if (not someTrue) {
                cout << "Error in model, clause is not satisfied:";
                for (unsigned int  j = 0; j < clauses2[i].size(); ++j) {
                    cout << clauses2[i][j] << " ";
                }
                cout << endl;
                exit(1);
     ...
     * @param satisfiable whether the problem was found to be satisfiable or not
     */
    void exitWithSatisfiability2(bool satisfiable) {
        if (satisfiable) {
            checkmodel();
            cout << "SATISFIABLE," <<endl;//<< decisions << "," << propagations << endl;
            exit(20);
    ...
    void executeDPLL2() {
        ...
        while (true) {
            ...
            if (decisionLit == 0) {
                cout<<"error2\n";
                exitWithSatisfiability2(true);
            }
            // start new decision level:
            modelStack2.push_back(DECISION_MARK);  // push mark indicating new
            ++indexOfNextLiteralToPropagate2;
            ++decisionLevel2;
            setLiteralToTrue2(decisionLit); // now push decisionLit on top of the mark
        }
    }
     ...
     * early failure is triggered.
     */
    void checkUnitClauses2() {
        for (unsigned int  i = 0; i < numClauses2; ++i) {
            if (clauses[i].size() == 1) {
                int literal = clauses[i][0];
                int value = currentValueForLiteral2(literal);
                if (value == FALSE) {
                    // This condition will only occur if at least a couple of unit clauses
                    //  exist with opposite literals (p and ¬p, for example). When the first
                    //  unit clause is used to set a value in the model (see the next condition),
...
int main(int argc, char* argv[]) {
	...
	belive_rev a1,a2;
        cout<<"Colocar creencia principal\n";
